...
head(ETF.prices)
```
Sve varijable kreću se u dozvoljenim rasponima te nema nedostajućih vrijednosti.
...
```
Dvije očite grupacije su obveznički te dionički fondovi s velikom razinom međusobne korelacije. Fondovi koji ne pripadaju ni jednoj od te dvije grupe su USO, GLD, i DBC. GLD predstavlja zlato koje se na tržištu ponaša bitno drukčije od obveznica i dionica. Dok USO predstavlja naftu, a DBC 'commodity' cijene generalo. Kako je nafta vrlo bitna sirovina postoji značajna korelacija između DBC i USO. Energetske dionice, XLE također pokazuju relativno visoku korelaciju s cijenama nafta i ostalih sirovina.
...
```
Prve tri komponente objašnjavaju oko 77%, a prve četiri komponente 81% varijance SKALIRANIH podataka.
...
```
Koljeno scree plota nalazi se u četvrtoj komponenti pa ćemo u daljnoj analizi koristiti prve tri komponente.
...
```
Prve dvije komponente jasno predstavljaju dvije grupe koje smo uočili na korelacijskoj matrici. 

Prva komponenta mjeri koliko je neki portfolio 'sličan' dioničkim fondovima što se očituje u velikim pozitivnim koeficijentima za dioničke fondove, malim koeficijentima za obezničke fondove čija je korelacija s dioničkim blizu nule, te negativnim koeficijentima za obvezničke fondove koji su negativno korelirani s dioničkim.

Druga komponenta na isti način opisuje drugu grupaciju fondova.

Dok treća komponenta opisuje kretanje, 'outliera' DBC, GLD, i USO za koje smo primjetili da se ne ponašaju u ni kao obveznice ni kao dionice.

Četvrta komponenta nema vrlo očitu interpretaciju kao prethodne tri.
...
```
Na grafu raspršenja najjasnije vidimo grupaciju dioničkih fondova koji su se smjestili u gornjem desnom kutu grafa. Obveznički fondovi su također odvojeni u svoj kluster u donjem lijevom kutu grafa. GLD je smješten daleko udaljen od dioničkih i obevzničkih grupa, dok su USO i DBC smješteni relativno blizu dioničkoj grupaciji.
...
axis(1, at = midpts, labels=colnames(ETF_returns[,2:p]), cex.axis=0.47) # shrinks axis labels
```
Loš performans prvog svojstvenog portfelja može biti objašnjen većim brojem i magnitudom negativnih koeficijenata. Općeniti trend sve imovine u ovom periodu je rastuć pa ti koeficijenti dugoročno oduzimaju dosta vrijednosti portfelja. Nadalje, kriza 2008. godine svela je vrijednost portfelja skoro na nulu od čega se portfelj jako teško oporavio, dok se drugi svojstveni portfelj ponašao puno stabilnije u krizi.
...
Prva dva faktora ovih metoda konceptualno su ista. Prvi opisuje dioničke fondove, dok drugi opisuje obvezničke. Ipak, razlikuju se u trećem faktoru. U metodi glavnih komponenti treći faktor 'specijaliziran' je za opisivanje međuovisnosti USO, GLD, te DBC fondova, tj. koeficijenti su magnitudom veliki samo za te fodove te, u manjoj mjeri veliki za fondove s značajnom mjerom korelacije s dotičnim fondovima. Primjenom MLE metode dobiva se treći faktor koji je nema vrlo jasnu interpretaciju, te definitivno nije 'specijaliziran' za USO, GLD, te DBC fondove. Ipak, iz rezidualnih matrica korelacije može se zaključiti da treći faktor MLE metode na neki način specijalizirano opisuje kretanje IYR i VNQ fonda jer su reziduali korelacija vezanih uz ta dva fonda izuzetno mali, te su specifične varijance tih fondova svedene gotovo na nulu. To nije slučaj kod metode glavnih komponenata koja najlošije opisuje baš korelacije ta dva fonda. MLE metoda već ustanovljenu 'unikatnost' GLD, USO, i DBC fondova izrazito loše opisuje, pa se vrijednosti reziduala tih korelacija kreću oko 0.6. Ako moramo usporediti ova dva modela, uz fiksni broj komponenata, nije jasno koji je bolji. Treći faktor metode glavnih komponenata je lakši za interpretaciju, te ako nam je bitna točnost modeliranja portfelja s velikim udjelom GLD, DBC, te USO fondova onda je ta metoda dakako bolja. No, ako su nam bitniji IYR i VNQ fondovi onda bolje koristiti MLE metodu. Ako moramo odabrati općenito bolji model, SSE rezidualne matrice metode glavnih komponenata mnogo je manji, pa bi se bilo razumno opredijeliti za taj model.
...
Najbolju interpretabilnost dobijemo primjenom varimax rotacije koja maksimizira varijancu koeficijenata faktora, tj. neke koeficijente pogura u 0 a druge u 1. Interpretacija prva dva faktora ostaje ista, dok je treći faktor puno jasnije 'specijaliziran' za IYR te VNQ fond.
...
print(fa3$loadings)
```
Za razliku od metode glavnih komponenti, u slučaju promjene broja faktora u metodi najveće izglednosti, promijenit će se i sve procjene koeficijenata faktorskog modela. Povećanjem broja faktora, sve bolje i bolje možemo opisati podatke pa tako i specifična varijanca u prosjeku opada. To uočavamo i na grafu. Najveći pad specifične varijance uočavamo za IYR te VNQ fondove, što je i za očekivati jer se faktor tri upravo za njih 'specijalizirao'.
...
legend("bottomleft", 
       ...
       cex = 0.65)
```
Za vrijeme krize prosječni povrati negativni su samo za dioničke fondove, dok za obvezničke i dalje ostaju pozitivni.
Vidimo da korelacija AGG fonda ima i vremensku komponentu u sebi jer se priroda većine korelacija s dioničkim fondovima u potpunosti promijeni za vrijeme krize. Dakle AGG fond postaje pozitivno koreliran s dioničkim fondovima za vrijeme krize, dok magnituda njegove korelacije s obvezničkim fondovima, tijekom krize, pada.
...
```
QDA daje bolje rezultate od LDA, što je i za očekivati jer je kompleksiniji klasifikator te se može bolje prilagoditi podatcima. LDA rezultira linearnom diskriminantom dok QDA rezultira kvadratnom. U ovom slučaju trebamo preferirati QDA jer je pretpostavka jednakosti kovarijanci grupa očito nije zadovoljena. Prikazali smo kako se priroda korelacija fonda AGG mijenja u ovisnosti o krizi.
...
print(apers)
```
Korištenjem manjeg broja komponenti gubimo dio informacija originalnog dataseta pa ne možemo toliko dobro prepoznati krizne i nekrizne tjedne. S povećanjem korištenih komponenata taj gubitak je sve manji, te kad koristimo svih 24 komponente postižemo jednaku APER mjeru kao i kad koristimo originalni dataset. Kad koristimo sve komponente gubitka informacije nema, pa je to zapravo ekvivalentno kao i da radimo s originalnim datasetom.
